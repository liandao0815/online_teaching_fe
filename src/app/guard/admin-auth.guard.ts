...
export class AdminAuthGuard implements CanActivate, CanActivateChild {
  ...
  canActivate(): Observable<boolean> | boolean {
    ...
    return this.userService.loadUserInfo().pipe(
      take(1),
      switchMap(this.loadAdminInfo),
      catchError(() => {
        this.message.remove(this.messageId);
        this.router.navigateByUrl('/admin/sign_in');
  ...
  canActivateChild(): boolean {
    return this.userService.adminLogin;
  }

  private loadAdminInfo = (res: ResponseData<User>): Observable<boolean> => {
    this.message.remove(this.messageId);
